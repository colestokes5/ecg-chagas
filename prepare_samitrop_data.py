...
def get_parser():
    ...
    parser.add_argument('-i', '--signal_file', type=str, required=True) # samitrop_exams.hdf5
    parser.add_argument('-d', '--demographics_file', type=str, required=True) # samitrop_exams.csv
...
# Run script.
def run(args):
    # Load the patient demographic input_data.
    exam_ids = list()
    exam_id_to_age = dict()
    exam_id_to_sex = dict()
    ...
    # Load and convert the signal input_data.
    ...
    with h5py.File(args.signal_file, 'r') as f:
        ...
        for i in range(num_exam_ids):
            ...
            # Add the exam ID, the patient ID, age, sex, Chagas label, and input_data source.
