...
def get_parser():
    ...
    parser.add_argument('-i', '--input_folder', type=str, required=True) # records500
...
# Run script.
def run(args):
    # Flatten the input directory: move all files from subdirectories into the root of input_folder
    for root, dirs, files in os.walk(args.input_folder):
        for file in files:
            full_path = os.path.join(root, file)
            if root != args.input_folder:  # Skip if already in the root
                dest_path = os.path.join(args.input_folder, file)
                if not os.path.exists(dest_path):
                    shutil.move(full_path, dest_path)

    # Remove now-empty subdirectories
    for root, dirs, files in os.walk(args.input_folder, topdown=False):
        for dir in dirs:
    ...
    # Update the header files to include demographics input_data and copy the signal files unchanged.
    for record in records:

        # Extract the demographics input_data.
        ...
        # Convert input_data from .dat files to .mat files as requested.
