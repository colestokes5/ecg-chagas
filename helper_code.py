...
### Challenge input_data I/O functions
...
# Save the models outputs for a record.
...
def get_label(string, allow_missing=False):
    ...
    if not has_label and not allow_missing:
        raise Exception('No label is available: are you trying to load the labels from the held-out input_data?')
...
def get_probability(string, allow_missing=False):
    ...
    if not has_probability and not allow_missing:
        raise Exception('No probability is available: are you trying to load the labels from the held-out input_data?')
...
def compute_challenge_score(labels, outputs, max_fraction_positive = 0.05):
    # Check the input_data.
    assert len(labels) == len(outputs)
    num_instances = len(labels)
    max_num_positive_instances = int(max_fraction_positive * num_instances)

    # Convert the input_data to NumPy arrays, as needed, for easier indexing.
    ...
    # Find the true positive rate so that the number of positive models outputs are no more than 5% of the total instances.
    k = num_thresholds
    for j in range(1, num_thresholds):
...
def compute_auc(labels, outputs):
    ...
    # Convert the input_data to NumPy arrays for easier indexing.
    labels = np.asarray(labels, dtype=np.float64)
    outputs = np.asarray(outputs, dtype=np.float64)
